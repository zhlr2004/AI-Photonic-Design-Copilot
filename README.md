...
`src/photonic_copilot/folder_example_library.py` 提供：
...
路径优先级为：显式参数 → `PHOTONIC_EXAMPLE_LIBRARY_ROOT` → 项目根目录
`example-library/`。例如：
...
library = ExampleLibrary()  # 默认 <project-root>/example-library
# 或 ExampleLibrary(Path(r"D:\PhotonicExampleLibrary"))
```
...
每个发布版本都是自包含且不可覆盖的目录，内部引用使用相对路径。
`ExampleManifest` 是权威记录，`catalog.json` 只是可通过
`rebuild_catalog()` 重建的轻量索引。
...
检查并清洗 @legacy-case/。保留源目录不变，将副本复制到样例库 staging，
删除多余缓存并补充可追溯字段，完成后生成 G3 审核包，不要自动发布。
```

清洗工具只修改 staging 副本。缺失的物理、执行、收敛、许可或审核信息
不得猜测；缺少 RunManifest/ValidationReport 的旧案例最高只能作为
`Archived` 候选。

#### 从已完成人工审核的案例开始入库

已有人工审核结论时，不必重新进行完整科学评审，但仍不能直接复制到
`examples/`。原因是格式清洗、相对路径重写或字段补全会产生新的候选
字节快照，原审核不能自动批准一个内容已经变化的版本。
...
将已经人工审核的 @reviewed-case/ 加入文件夹样例库。
已有审核文件为 @reviewed-case/human-review.md。

要求：
1. 保持源案例只读并计算 SourceSnapshot；
2. 保存原人工审核作为 provenance；
3. 复制到 staging，清理多余文件并补全可追溯字段；
4. 运行 Schema、哈希、许可和敏感信息检查；
5. 生成最终 ExampleCandidate SHA-256；
6. 展示清洗前后变化，请审核人确认该规范化候选；
7. 生成绑定最终候选哈希的 G3ReviewDecision；
8. 未得到确认前不要发布。
```
...
```text
python curate-photonic-example-case/scripts/publish_reviewed.py \
  --candidate-id <candidate-id> \
  --decision <g3-review-decision.json> \
  --library-root <example-library-root>
```

如果案例具有真实的执行/验证证据，可同时传入：
...
- 仅做过文档/人工审核但没有求解器 ValidationReport：只能发布为
  `Archived`；
- 有真实执行记录但没有收敛：最高 `Executed`；
- ValidationReport 为 validated 且 G3 批准：可以 `Validated`；
- 领域专家在规范化后确认最终候选：可以 `Reviewed`。
...
- 复现目标或设计目标；
- 材料、偏振、边界和模式；
- 单 MPI 进程或多 MPI 进程，以及准确进程数；
- 历史案例引用；
- 所有影响物理解释的假设；
- 验收指标和收敛方案。

MPI 选择属于 G1，而不是 G2：Agent 必须在编写 Meep Python 或
Lumerical Python/LSF 仿真代码前询问并记录。G2 负责批准是否真正执行
已经确定资源配置的代码。
...
在编写任何仿真代码前，询问我采用单 MPI 进程还是多 MPI 进程；
记录 execution_mode、mpi_processes 和 mpi_launcher。
先不要运行仿真。
```
...
6. 拟采用的求解方法和候选求解器；
7. 询问并记录单 MPI 进程或多 MPI 进程、准确进程数和 launcher；
8. mesh、PML 和 run-control 收敛计划；
9. 验收指标与阈值；
10. 预期输出工件。
...
采用多 MPI 进程，mpi_processes=8，launcher=mpiexec；
目标 panel 和验收阈值保持不变。
...
如果准备入库的是旧目录或非标准案例，必须先调用：

```text
@curate-photonic-example-case/SKILL.md

将 @legacy-paper-case/ 作为 paper_reproduction 案例导入。
只读盘点源目录，展示清洗计划；经我确认后复制到 staging，
仅清洗副本并补充可追溯信息。生成 ExampleCandidate 和 G3 审核包，
不要修改源目录，也不要自动发布。
```

G3 应审核清洗后的候选快照；候选改变后原批准失效。
...
如果案例在进入框架前已经完成人工审核，使用上文“从已完成人工审核的
案例开始入库”流程，将原审核保存为 provenance，并在清洗后确认新的
候选哈希。
...
在编写仿真代码前询问我采用单 MPI 进程还是多 MPI 进程，
不要立即运行求解器。
```
...
并行方式：single_mpi_process 或 multi_mpi_process、进程数和 launcher
...
7. 询问并记录单 MPI 进程或多 MPI 进程、准确进程数和 launcher；
8. mesh、PML 和 run-control 收敛案例；
9. 资源预算和最大仿真次数；
10. 候选求解器及 capability 匹配结果。
...
如果来源是旧的自由设计目录或工件格式不符合当前 Schema，先输入：

```text
@curate-photonic-example-case/SKILL.md

将 @legacy-design-case/ 作为 free_design 案例导入。
只读检查并生成清洗计划；保留源目录不变，将副本放入 staging。
删除副本中的缓存和无关文件，补充可计算/可追溯字段，
保留父案例和参数变换，输出 ExampleCandidate 和 G3 审核包。
缺失的运行或收敛证据不得伪造。
```
...
若该自由设计案例已有外部人工审核，先保存原审核并完成 staging
规范化；审核人必须确认规范化后的 ExampleCandidate 哈希，再使用
`publish_reviewed.py` 发布。
